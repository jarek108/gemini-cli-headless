...
1. **Scoping & Encapsulation:** If you delete the dds/ directory, you delete the project and all its active workers cleanly. There is no orphaned state left in a global sandboxes folder.
...
- **Hardware Mutex:** alse. This project is lightweight. Parallel instances are fully supported.
`
...
For example, the Manager generates a un_config.json inside instance_worker_1/.agent_run/ that includes:
`json
{

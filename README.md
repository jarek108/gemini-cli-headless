...
*   **[🏗️ The Hierarchical Architecture & Execution Loop](docs/vision_architecture.md)**: Details the Manager/Worker topology, the Artifact-Driven Workflow (`IRP.md`, `QRP.md`), and the `agent_loop.py` deterministic execution engine.
*   **[🎬 Interaction Scenarios: A Day in the Life](docs/vision_scenarios.md)**: Practical examples of how a human (CEO) interacts with the Manager CLI to delegate tasks, monitor progress via passive hooks, and handle deadlock supervision.